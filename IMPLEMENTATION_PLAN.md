...
| `typescript` | **5.9.3** (fallback confirmado; ver detalle debajo de la tabla) | `latest` es `7.0.2`, se usa `ts5.9`/`prev` | Ver "Decisión: TypeScript 5.9.3 en vez de 7.0.2" justo debajo de esta tabla |
...
**PostgreSQL**: se usa **16.x** como referencia (el entorno de desarrollo trae `postgresql-16` instalado de forma nativa, y es la versión típica de las plantillas gestionadas de Railway). Prisma 7.10.0 soporta también PostgreSQL 15 y 17, por lo que la aplicación no queda anclada a una minor exacta: cualquier PostgreSQL ≥ 14 soportado por Railway es válido. La conexión se hace exclusivamente por `DATABASE_URL` estándar (driver adapter `@prisma/adapter-pg` + `pg`, ver §4.4.1) contra el PostgreSQL que provisiona el propio proyecto de Railway — **no** hay dependencia de Prisma Postgres ni de Prisma Compute (ambos son el servicio de base de datos gestionada propio de Prisma, un producto distinto y no requerido por este proyecto).

**Node.js**: el stack actual (Next 16.3.4, Prisma **7**.10.0, TypeScript 5.9.3, Vitest 5.0.0) solo requiere Node `^20.19 || ^22.12 || >=24.0` (rango exacto de Prisma 7, el más estricto de los cuatro). El entorno de desarrollo usado para construir y verificar la Fase 1 corre **Node 22.22.2**, que cumple ese rango sin ningún ajuste. No se fija `engines.node` a `24.x` en `package.json`: nada en el stack lo exige hoy, y forzarlo excluiría innecesariamente Node 22 (Active LTS) sin ningún beneficio real mientras se siga en Prisma 7. Si en el futuro el proyecto migra a Prisma 8 (cuando exista una versión estable) y esa versión exige Node 24+, este apartado se actualiza junto con esa migración.

### Decisión: Prisma 7.10.0 (no Prisma 8)

Verificado dos veces en el mismo proyecto — al elegir la matriz de versiones (§3.3, más arriba) y de nuevo explícitamente a pedido, re-consultando el registro de npm en vivo con `npm view prisma dist-tags` y `npm view prisma versions`:

- **Versión inicialmente evaluada**: `prisma@latest` según el dist-tag del paquete, que apunta a `8.0.0-rc.13`.
- **Incompatibilidad/problema encontrado**: no es un problema de compatibilidad de código, sino de estabilidad del release. El dist-tag `latest` de dos paquetes hermanos del mismo proyecto es inconsistente (`prisma` → `8.0.0-rc.13`, `@prisma/client` → `7.10.0`), y una revisión completa del historial de versiones publicadas de `prisma` en npm (54 versiones de la serie `8.x` a la fecha de esta verificación) confirma que **todas** llevan sufijo `-rc.*` o `-dev.*` — no existe ninguna versión `8.0.0` (ni `8.x.x`) publicada sin sufijo, es decir, **Prisma 8 todavía no tiene un release estable (GA) en el registro de npm**, solo release candidates.
- **Fallback estable seleccionado**: **Prisma 7.10.0** (CLI y `@prisma/client`), la última versión de la serie 7 sin sufijo de pre-release, confirmada como estable e instalada (`npx prisma --version` → `prisma: 7.10.0`, `@prisma/client: 7.10.0`).
- **Motivo**: aplica directamente el principio ya acordado para todo el proyecto — "evitar versiones beta/RC/canary/experimentales salvo necesidad técnica claramente justificada" (§82.3 del encargo original). No hay ninguna necesidad técnica que obligue a adoptar una release candidate: Prisma 7.10.0 cubre todos los requisitos (driver adapter para PostgreSQL vía `DATABASE_URL`, migraciones, tipado end-to-end) sin el riesgo de estabilidad de una RC. Se reevaluará en cuanto npm publique una versión `8.x` sin sufijo `-rc`/`-dev`.

Sobre archivos o *skills* de Prisma 7: en ningún momento se generaron dentro de este repositorio. Al investigar el flujo de configuración de Prisma 7 (`prisma.config.ts`, generador `prisma-client`, driver adapters — todo esto es real y confirmado por ejecución, ver §4.4.1) se usó un directorio de trabajo temporal **fuera del repositorio** (`/tmp/prisma-init-test`) para inspeccionar qué genera `npx prisma init`, incluidas unas *skills* de agente (`.claude/skills/prisma-*`) que el propio CLI de Prisma instala como documentación de referencia. Esos archivos nunca se copiaron ni se comitearon a este proyecto — `git status` no muestra ningún directorio de *skills* en el repositorio real, solo el `AGENTS.md`/`CLAUDE.md` generados por `create-next-app` (de Next.js, no de Prisma) que ya estaban commiteados desde la Fase 1.

### Decisión: TypeScript 5.9.3 en vez de 7.0.2

- **Versión inicialmente evaluada**: `typescript@7.0.2` (dist-tag `latest`), el compilador nativo reescrito ("tsc-go"), por ser la versión estable más reciente publicada.
- **Incompatibilidad encontrada**: al ejecutar `npx eslint .` sobre el proyecto ya scaffolded, `typescript-eslint` (dependencia de `eslint-config-next`) abortó con error en tiempo de ejecución: *"typescript-eslint does not support TS 7.0 [...] see tracking issue for support of TS >=7.1"*. Es una incompatibilidad real, confirmada por ejecución directa del comando — no una precaución especulativa.
- **Fallback estable seleccionado**: **TypeScript 5.9.3**, la última versión estable de la línea clásica del compilador (previa a la serie 6.x/7.x), sin ningún sufijo de pre-release.
...
- `manifest.webmanifest`: se genera con la convención nativa de Next.js (`src/app/manifest.ts`, tipado con `MetadataRoute.Manifest`), no como archivo estático — Next lo sirve y lo enlaza automáticamente en `<head>`. Incluye nombre, iconos (192/512 "any" + 512 "maskable" con zona de seguridad), `display: standalone`, `theme_color`, `start_url`.
- Iconos y favicon: se generan bajo demanda con `ImageResponse` de `next/og` (`src/app/icons/[size]/route.tsx`, `src/app/icon.tsx`, `src/app/apple-icon.tsx`), leyendo el logo maestro (`public/brand/logo.png`, ver README.md §"Marca") vía `src/lib/server/brandAssets.ts` — sin depender de herramientas externas de generación de íconos ni de binarios. Sin ese archivo (repositorio base, antes de subir el logo definitivo), cae al placeholder original (una marca simple con el color de marca).
