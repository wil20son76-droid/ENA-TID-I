...
async function loadBatchEvents(batchId: string) {
  ...
  return { stockings, transfers, mortalities };
}
...
export async function getBatchDistribution(batchId: string): Promise<Record<string, number>> {
  const { stockings, transfers, mortalities } = await loadBatchEvents(batchId);
  return calculateBatchDistribution(stockings, transfers, mortalities, batchId);
}
...
export async function getBatchTotalQuantity(batchId: string): Promise<number> {
  const { stockings, transfers, mortalities } = await loadBatchEvents(batchId);
  return calculateBatchTotalBalance(stockings, transfers, mortalities, batchId);
}
...
export async function getPondOccupancy(pondId: string): Promise<Record<string, number>> {
  const [allStockings, allTransfers, allMortalities] = await Promise.all([
  ...
  // mortalidad y se deja que la función de dominio filtre por pondId.
  ...
  return calculatePondOccupancy(relevantStockings, relevantTransfers, relevantMortalities, pondId);
}
...
/** Historial de un estanque (siembras + traslados + mortalidad + alimentación + muestreos). */
export async function getPondHistory(pondId: string): Promise<PondHistoryEvent[]> {
  const [stockings, transfersIn, transfersOut, mortalities, feedings, samplings] =
    await Promise.all([
      db.stockings.where("pondId").equals(pondId).toArray(),
